...
import { Server, Menu, X, Zap } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex space-x-8">
            {['VPS Hosting', 'Dedicated', 'Minecraft', 'Limited Deals'].map((text, index) => (
              <motion.div
                key={text}
              ...
              >
                <NavLink 
                  ...
                  text={text} 
                  icon={text === 'Limited Deals' ? <Zap className="w-4 h-4" /> : null}
                />
              </motion.div>
            ))}
          </div>
...
const MobileNavLink = ({ to, text, onClick, icon }) => (
  <motion.div
    whileHover={{ x : 10 }}
    transition={{ type: "spring", stiffness: 400, damping: 10 }}
  >
    <Link
      to={to}
      onClick={onClick}
      className="block text-gray-300 hover:text-purple-400 transition-colors py-2 text-lg flex items-center space-x-2"
    >
      {icon && <span>{icon}</span>}
      <span>{text}</span>
    </Link>
